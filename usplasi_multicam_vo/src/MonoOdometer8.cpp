#include <multicam_vo/MonoOdometer8.h>

/** Default constructor. */
MonoOdometer8::MonoOdometer8()
{

}

/** Constructor. */
MonoOdometer8::MonoOdometer8(Eigen::Matrix3f K) : node_("~")
{
...
std::vector<Match> MonoOdometer8::normalize2DPoints(std::vector<Match> matches, Eigen::Matrix3f &NormTPrev, Eigen::Matrix3f &NormTCurr)
{
    ...
    for(int i=0; i<matches.size(); i++)
    {
        ...
        pointsPrev.push_back(pPrev);
        pointsCurr.push_back(pCurr);
    }
    ...
    for(int i=0; i<matches.size(); i++)
    {
        sumPrev += pointsPrev[i];        
        sumCurr += pointsCurr[i];
    }
    ...
    for(int i=0; i<matches.size(); i++)
    {
        pointsPrev[i] = pointsPrev[i] - sumPrev;
        pointsCurr[i] = pointsCurr[i] - sumCurr;            
    }
    ...
    for(int i=0; i<matches.size(); i++)
    {
        cv::Point2f pt1 = pointsPrev[i];
        cv::Point2f pt2 = pointsCurr[i];
    ...
    for(int i=0; i<matches.size(); i++)
    {
        cv::Point2f pt1 = pointsPrev[i];
        cv::Point2f pt2 = pointsCurr[i];
        pointsPrev[i] = scalePrev * pt1;
        pointsCurr[i] = scaleCurr * pt2;

        matches[i].pPrev_ = pointsPrev[i];
        matches[i].pCurr_ = pointsCurr[i];
    }
...
Eigen::Matrix3f MonoOdometer8::getF(std::vector<Match> matches, std::vector<int> indices)
{
    int N = indices.size();
...
std::vector<int> MonoOdometer8::getInliers(std::vector<Match> matches, Eigen::Matrix3f F)
{
    std::vector<int> inlierIndices;
...
Eigen::Matrix3f MonoOdometer8::F2E(Eigen::Matrix3f F)
{
 ...
 * @return void */
void MonoOdometer8::E2Rt(Eigen::Matrix3f E, std::vector<Match> matches, Eigen::Matrix3f &R, Eigen::Vector3f &t, Eigen::Matrix<float, 4, Eigen::Dynamic> &points3D)
{
    Eigen::Matrix3f W, Z;
    W << 0, -1, 0, 1, 0, 0, 0, 0, 1;
    Z << 0, 1, 0, -1, 0, 0, 0, 0, 0;
...
int MonoOdometer8::triangulate(std::vector<Match> matches, Eigen::Matrix3f R, Eigen::Vector3f t, Eigen::Matrix<float, 4, Eigen::Dynamic> &points3D)
{
...
Eigen::Matrix<float, 3, 4> MonoOdometer8::getProjectionMatrix(Eigen::Matrix3f K, Eigen::Matrix3f R, Eigen::Vector3f t)
{
...
std::vector<Eigen::Vector4f> MonoOdometer8::getClosePoints(std::vector<Eigen::Vector4f> points, double &median)
{
...
double MonoOdometer8::getTranslationScale(std::vector<Eigen::Vector4f> points3D, double median, double pitch, double height)
{
