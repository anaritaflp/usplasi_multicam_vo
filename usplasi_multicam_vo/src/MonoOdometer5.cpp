#include <multicam_vo/MonoOdometer5.h>

/** Default constructor. */
MonoOdometer5::MonoOdometer5()
{

}

/** Constructor. */
MonoOdometer5::MonoOdometer5(Eigen::Matrix3f K) : node_("~")
{
...
std::vector<Match> MonoOdometer5::normalize2DPoints(std::vector<Match> matches, Eigen::Matrix3f &NormTPrev, Eigen::Matrix3f &NormTCurr)
{
...
Eigen::Matrix3f MonoOdometer5::F2E(Eigen::Matrix3f F)
{
...
Eigen::Matrix<float, 3, 4> MonoOdometer5::getProjectionMatrix(Eigen::Matrix3f K, Eigen::Matrix3f R, Eigen::Vector3f t)
{
...
std::vector<Eigen::Vector4f> MonoOdometer5::getClosePoints(std::vector<Eigen::Vector4f> points, double &median)
{
...
bool MonoOdometer5::testE(cv::Mat E)
{
    // multiply fundamental matrix by intrinsic matrices to obtain essential matrix
    Eigen::Matrix3f EEigen;
    EEigen << E.at<double>(0,0), E.at<double>(0,1), E.at<double>(0,2),
              E.at<double>(1,0), E.at<double>(1,1), E.at<double>(1,2),
              E.at<double>(2,0), E.at<double>(2,1), E.at<double>(2,2);

    Eigen::JacobiSVD<Eigen::Matrix3f> svdE(EEigen, Eigen::ComputeFullU | Eigen::ComputeFullV);

     if(std::abs(svdE.singularValues()(0) - svdE.singularValues()(1)) < 0.0005)
     {
         return true;
     }
     else
