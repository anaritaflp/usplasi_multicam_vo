...
class MulticamOdometer
{
    public:
		...
		* @param std::vector<Eigen::Matrix4f>& output vector with each camera's pose estimation, obtained from monocular VO
		...
		Eigen::Matrix4f estimateMotion(std::vector<std::vector<Match>> matches, int &bestCamera, std::vector<std::vector<Match>> &inlierMatches, std::vector<std::vector<Eigen::Vector3f>> &points3D, std::vector<Eigen::Matrix4f> &monoPoses, ros::Time timestamp);
		...
		* @return Eigen::Matrix3f fundamental matrix */
		...
		int getNumInliers(std::vector<Match> matches, Eigen::Matrix3f R, Eigen::Vector3f t, Eigen::Matrix3f K, double threshold);
		int checkChierality(std::vector<Match> matches, Eigen::Matrix3f rotation, Eigen::Vector3f translation);
    ...
    private:
		...
		std::vector<MonoOdometer8> monoOdometers_;
		std::vector<ros::Publisher> pubMonoOdoms_;
