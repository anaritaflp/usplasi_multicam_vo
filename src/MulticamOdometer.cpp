...
MulticamOdometer::MulticamOdometer(ros::NodeHandle node): node_(node)
{
    ...
    node.getParam("param_extrinsicsCam0", param_extrinsicsCam0_);
    node.getParam("param_extrinsicsCam1", param_extrinsicsCam1_);
    node.getParam("param_extrinsicsCam2", param_extrinsicsCam2_);
    node.getParam("param_extrinsicsCam3", param_extrinsicsCam3_);
    node.getParam("param_extrinsicsCam4", param_extrinsicsCam4_);
    node.getParam("param_extrinsicsCam5", param_extrinsicsCam5_);
}
...
Eigen::Matrix4f MulticamOdometer::estimateMotion(std::vector<std::vector<Match>> matches, int numCameras, std::vector<image_geometry::PinholeCameraModel> intrinsics)
{
    ...
    for(int i=0; i<3*numCameras; i++)
    {
        ...
        Eigen::Matrix3f KPrev, KCurr;
        KPrev << intrinsics[camNoPrev].fx(), 0.0, intrinsics[camNoPrev].cx(), 0.0, intrinsics[camNoPrev].fy(), intrinsics[camNoPrev].cy(), 0.0, 0.0, 1.0;
        KCurr << intrinsics[camNoCurr].fx(), 0.0, intrinsics[camNoCurr].cx(), 0.0, intrinsics[camNoCurr].fy(), intrinsics[camNoCurr].cy(), 0.0, 0.0, 1.0;
    ...
    // transform to front camera (0) coordinates
    Eigen::Matrix4f cam0Ext = transform2ExtrinsicsVector(param_extrinsicsCam0_);
    Eigen::Matrix4f cam1Ext = transform2ExtrinsicsVector(param_extrinsicsCam1_);
    Eigen::Matrix4f cam2Ext = transform2ExtrinsicsVector(param_extrinsicsCam2_);
    Eigen::Matrix4f cam3Ext = transform2ExtrinsicsVector(param_extrinsicsCam3_);
    Eigen::Matrix4f cam4Ext = transform2ExtrinsicsVector(param_extrinsicsCam4_);
...
{
    // get rotation matrix
    double rx = extrinsics[0];
    double ry = extrinsics[1];
    double rz = extrinsics[2];
    Eigen::Matrix3 Rx, Ry, Rz;
    ...
    Eigen::Vector3f t;
    t << extrinsics[3], extrinsics[4], extrinsics[5];
