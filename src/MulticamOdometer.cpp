...
Eigen::Matrix4f MulticamOdometer::estimateMotion(std::vector<std::vector<Match>> matches, int &bestCamera, std::vector<std::vector<Match>> &inlierMatches, std::vector<std::vector<Eigen::Vector3f>> &points3D)
{
    ...
    inlierMatches.resize(3 * NUM_OMNI_CAMERAS);
    points3D.resize(3 * NUM_OMNI_CAMERAS);
    ...
    for(int i=0; i<3*NUM_OMNI_CAMERAS; i++)
    {
        ...
        success[i] = monoOdometer.estimateMotion(matches[i], KPrev, KCurr, R[i], t[i], false, inlierMatches[i], points3D[i]);
    }
